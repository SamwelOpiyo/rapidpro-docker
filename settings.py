...
INSTALLED_APPS = INSTALLED_APPS + tuple(
    filter(None, env("EXTRA_INSTALLED_APPS", "").split(","))
)
...
if AWS_STORAGE_BUCKET_NAME:
    ...
    if AWS_MEDIA:
        MEDIA_URL = "https://s3.amazonaws.com/%s/media/" % (
...
COMPRESS_OFFLINE_MANIFEST = "manifest-%s.json" % env(
    "RAPIDPRO_VERSION", required=True
)
...
SECURE_PROXY_SSL_HEADER = (
    env("SECURE_PROXY_SSL_HEADER", "HTTP_X_FORWARDED_PROTO"),
    "https",
)
...
BRANDING = {
    "rapidpro.io": {
        ...
        "colors": dict(
            [
                rule.split("=")
                for rule in env("BRANDING_COLORS", "primary=#0c6596").split(
                    ";"
                )
            ]
        ),
        ...
        "credits": _(
            "Copyright &copy; 2012-%s UNICEF, Nyaruka, and individual contributors. All Rights Reserved."
